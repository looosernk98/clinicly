...
import { AppContextType, Appointment, RazorpayOrder } from '../../types'
...
const MyAppointments: React.FC = () => {
  ...
  const initPay = (order: RazorpayOrder): void => {
  ...
  const appointmentRazorpay = async (appointmentId: string): Promise<void> => {
  ...
  const appointmentStripe = async (appointmentId: string): Promise<void> => {
    try {
        ...
        {appointments.map((item, index) => (
          <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
            ...
            <div className='flex-1 text-sm text-[#5E5E5E]'>
              ...
              <p className=' mt-1'><span className='text-sm text-[#3C3C3C] font-medium'>Date & Time:</span> {slotDateFormat(item.slot_date)} |  {item.start_time}</p>
            </div>
            <div></div>
